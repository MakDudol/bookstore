# «Солов'їна» — демо-магазин книжок

Цей репозиторій містить демо-версію інтернет-магазину «Солов'їна», створену на React + Vite. Усі стилі написані звичайним CSS, а дані беруться з простого масиву у `src/data/books.js`. Макет призначений для швидких показів та подальшого розширення.
...
- `src/data/books.js` — демо-список видань «Кобзаря» для вітрини «Солов'їна».
- `src/utils/storage.js` — збереження/читання кошика з `localStorage`.
- `src/assets/logo.png` - логотип для хедера.
